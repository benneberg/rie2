...
class ChatService {
  ...
  async saveDocumentation(documentation: Record<string, string>): Promise<ChatResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/save-docs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      return { success: false, error: 'Failed to save documentation' };
    }
  }
