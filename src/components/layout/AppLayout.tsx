    ...
    <div className={cn("min-h-screen flex flex-col relative selection:bg-primary/30 selection:text-primary", className)}>
      ...
      <header className="fixed top-0 left-0 right-0 z-[60] h-16 glass border-b border-white/5 px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          {sessionId && (
            <div className="px-3 py-1 bg-white/5 border border-white/5 rounded text-[10px] font-mono opacity-50 uppercase tracking-widest hidden sm:block">
      ...
      <main className={cn("flex-1 pt-16 pb-24 relative z-10", contentClassName)}>
        <div className={cn(container && "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12")}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[60] h-16 glass border-t border-white/5 px-8 flex items-center justify-between">
        <div className="flex gap-8 items-center">
          <Link to="/" className="text-[10px] font-black uppercase tracking-[0.2em] hover:text-primary transition-colors">Terminal</Link>
        ...
        {isMenuOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-[110] bg-[#070911]/90 backdrop-blur-md"
            />
            <motion.div
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              ...
              className="fixed top-0 right-0 bottom-0 w-full max-w-md z-[120] bg-[#0b0e18] border-l border-white/10 p-0 flex flex-col relative overflow-hidden"
            >
                    ...
                    <div className="flex flex-col gap-4">
                      {sessionId && navLinks.map((link) => (
                        <Link
                          ...
                          onClick={() => setIsMenuOpen(false)}
                          className="flex items-center justify-between text-2xl font-display font-black hover:text-primary transition-all group border-b border-white/5 pb-2"
                        >
