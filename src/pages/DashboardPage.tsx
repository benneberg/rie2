...
import { AlertTriangle, Github, HardDrive, LayoutGrid, Activity, Wrench, BarChart3, PieChart as PieChartIcon, Info, Cpu, Globe, Terminal } from 'lucide-react';
...
export function DashboardPage() {
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'firmware': return <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/30 text-[10px] font-bold uppercase tracking-widest"><Cpu className="w-3 h-3 mr-1" /> FIRMWARE</Badge>;
      case 'web': return <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/30 text-[10px] font-bold uppercase tracking-widest"><Globe className="w-3 h-3 mr-1" /> WEB_APP</Badge>;
      case 'cli': return <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px] font-bold uppercase tracking-widest"><Terminal className="w-3 h-3 mr-1" /> CLI_TOOL</Badge>;
      default: return <Badge variant="outline" className="text-[10px] font-bold tracking-widest uppercase">GENERAL</Badge>;
    }
  };
  return (
    <AppLayout container>
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-reveal">
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 items-center">
              <Badge variant="outline" className="text-[10px] font-bold tracking-widest border-primary/30 text-primary uppercase">Report_V4.2.1</Badge>
              {metadata.isMonorepo && <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30 text-[10px] font-bold uppercase tracking-widest">MONOREPO</Badge>}
              {getTypeBadge(projectType)}
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <Info className="w-4 h-4 text-white/20 hover:text-cyan-400 transition-colors" />
                    </TooltipTrigger>
                    <TooltipContent className="bg-midnight border-white/10 text-[10px] font-mono p-4 max-w-xs uppercase leading-relaxed">
                      Deterministic score based on RIE Engine v4.2 analysis of file structure, coupling, and security leaks.
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
          ...
          <Card className="glass border-l-4 border-l-emerald-500 shadow-brutal-dark">
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-widest">Coupling_Weights</h3>
              </div>
              <div className="h-[200px]">
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="glass border-l-4 border-l-emerald-500">
                <CardContent className="p-6">
                   <div className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-2 flex items-center gap-2">Isolation_Score</div>
              ...
              <Card className="glass border-l-4 border-l-amber-500">
                <CardContent className="p-6">
                   <div className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-2 flex items-center gap-2">Coupling_Index</div>
